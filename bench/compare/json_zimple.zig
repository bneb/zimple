...
fn expect(ctx: *Ctx, word: []const u8) ParseError!void {
    if (ctx.pos + word.len > ctx.input.len or !std.mem.eql(u8, ctx.input[ctx.pos..ctx.pos+word.len], word)) return error.InvalidToken;
    ctx.pos += word.len;
}

fn parseNull(ctx: *Ctx) ParseError!Value { try expect(ctx, "null"); return .{ .null_val = {} }; }
fn parseBool(ctx: *Ctx, val: bool) ParseError!Value { try expect(ctx, if (val) "true" else "false"); return .{ .bool_val = val }; }
